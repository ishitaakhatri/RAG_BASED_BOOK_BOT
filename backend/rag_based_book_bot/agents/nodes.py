...
class QueryAnalysis(BaseModel):
    ...
    topics: List[str] = Field(..., description="Key technical concepts")
    keywords: List[str] = Field(..., description="Search terms for retrieval")
    code_language: Optional[str] = Field(None, description="Programming language referenced")
    complexity_hint: str = Field("intermediate", description="beginner, intermediate, advanced")

class ContextAnalysis(BaseModel):
    needs_conversation_context: bool = Field(..., description="Does query use pronouns or implicit refs?")
    references_turn: Optional[int] = Field(None, description="Which turn number is referenced?")
    can_answer_from_history: bool = Field(..., description="Can this be answered from history?")
    standalone_query: str = Field(..., description="Rewritten standalone query")

class RewrittenQueries(BaseModel):
    variations: List[str] = Field(..., description="List of 3 alternative queries")
...
def query_parser_node(state: AgentState) -> dict:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("human", """Analyze the user query for a coding assistant.
        Intents: conceptual, code_request, debugging, comparison, tutorial.
        Return strict JSON.
        {format_instructions}
        User Query: {query}""")
    ])
    ...
    try:
        ...
        print(f"  ⚠️ Parsing failed, using default: {e}")
        return {
            "parsed_query": ParsedQuery(raw_query=state.user_query, intent=QueryIntent.CONCEPTUAL),
            "errors": [str(e)]
        }
...
def context_resolution_node(state: AgentState) -> dict:
    ...
    if not state.conversation_history:
        print(f"\n[Context Resolution] No history.")
        return {"resolved_query": current_query, "needs_retrieval": True}
        
    print(f"\n[Context Resolution] Analyzing history...")
    llm = get_llm()
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("human", """Given conversation history, determine if the new query needs context.
        Provide a standalone query.
        Return strict JSON.
        {format_instructions}
        History: {history}
        Query: {query}""")
    ...
    try:
        ...
        return {"resolved_query": current_query, "needs_retrieval": True, "errors": [str(e)]}
...
def answer_from_history_node(state: AgentState) -> dict:
    ...
    if state.referenced_turn and 0 <= state.referenced_turn - 1 < len(state.conversation_history):
        turns = [state.conversation_history[state.referenced_turn - 1]]
        
    history_context = "\n\n".join([f"Q: {t.user_query}\nA: {t.assistant_response}" for t in turns])
    
    prompt = ChatPromptTemplate.from_messages([
        ("human", """Answer solely based on this history.
        History: {history}
        Question: {question}""")
    ])
    ...
    try:
        ...
        return {
            "response": LLMResponse(answer=response.content, confidence=0.8),
            "pipeline_snapshots": [{"stage": "memory_answer", "chunk_count": 0}]
        }
    except Exception as e:
        return {"errors": [str(e)]}

def query_rewriter_node(state: AgentState) -> dict:
    q = state.resolved_query or state.user_query
    print(f"\n[Query Rewriting] {q}")
    llm = get_llm()
    parser = PydanticOutputParser(pydantic_object=RewrittenQueries)
    
    prompt = ChatPromptTemplate.from_messages([
        ("human", """Generate 3 search variations. Return strict JSON.
        {format_instructions}
        Query: {query}""")
    ])
    ...
    try:
        ...
        return {"rewritten_queries": res.variations}
    except:
        return {"rewritten_queries": [q]}
...
def vector_search_node(state: AgentState) -> dict:
    ...
    try:
        ...
        filter_dict = {}
        if state.book_filter: filter_dict["book_title"] = state.book_filter
        if state.chapter_filter: filter_dict["chapter_numbers"] = {"$in": [state.chapter_filter]}

        for q in queries:
            if not q: continue
            emb = model.encode(q).tolist()
            res = index.query(
                vector=emb, top_k=state.pass1_k, namespace=NAMESPACE, 
                include_metadata=True, filter=filter_dict if filter_dict else None
            )
        ...
        for m in sorted_matches:
            ...
            chunks.append(RetrievedChunk(
                chunk=DocumentChunk(
                    chunk_id=m['id'],
                    content=md.get('text', ''),
                    chapter=md.get('chapter_title', md.get('chapter', 'Unknown')),
                    book_title=md.get('book_title', 'Unknown'),
        ...
        return {
            ...
            "pipeline_snapshots": [{"stage": "vector_search", "chunk_count": len(chunks), "chunks": chunks[:10]}]
        }
    except Exception as e:
        return {"errors": [f"Search failed: {e}"]}

def reranking_node(state: AgentState) -> dict:
    print(f"\n[PASS 2] Reranking")
    if not state.retrieved_chunks: return {}
    
    try:
        reranker = get_cross_encoder()
        query = state.resolved_query or state.user_query
        to_rank = [{'text': rc.chunk.content, 'metadata': {'chunk_id': rc.chunk.chunk_id}} for rc in state.retrieved_chunks]
        ...
        return {
            ...
            "pipeline_snapshots": [{"stage": "reranking", "chunk_count": len(reranked), "chunks": reranked[:10]}]
        }
    except Exception as e:
        return {"errors": [f"Rerank failed: {e}"]}

def context_assembly_node(state: AgentState) -> dict:
    context = ""
    for rc in state.reranked_chunks:
        context += f"Source: {rc.chunk.book_title} (Page {rc.chunk.page_number})\n{rc.chunk.content}\n---\n"
    
    return {
        "assembled_context": context,
        "pipeline_snapshots": [{"stage": "assembly", "chunk_count": len(state.reranked_chunks)}]
    }
...
def llm_reasoning_node(state: AgentState) -> dict:
    ...
    if not state.assembled_context:
        return {"response": LLMResponse(answer="I couldn't find relevant info.", confidence=0.0)}
